...
//               <p className="w-1/6 text-center font-bold">{notification.createAt}</p>
//             </div>
//           ))}
//         </div>
//       </div>
...
import { useEffect, useState } from "react";
import { getTableNotifications } from "../services/notificationService";
...
const NotificationList = () => {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        const data = await getTableNotifications(); // Gọi API từ notificationService
        setNotifications(data);
      } catch (error) {
        console.error("Error fetching notifications:", error);
      } finally {
