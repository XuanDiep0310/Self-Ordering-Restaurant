...
import { useState } from "react";
import { updateStaffInfo } from "../../services/staffService"; // Nếu muốn test API thật thì giữ lại, nếu không có thể comment
import InputField from "../../components/Admin/InputField";
import Sidebar from "./Sidebar";
import AdminHeader from "../../components/Admin/Admin_header";
import { logout } from "../../services/authService";

const AdminInfo = ({ onClose }) => {
  // Dữ liệu cứng thay cho props user
  const hardcodedUser = {
    adminId: 1,
    ...
    password: "Admin@1234" // Trống mặc định
  };
  ...
  const handleSave = async () => {
    try {
      ...
      // Nếu chỉ test frontend, bạn có thể comment dòng dưới:
      // const response = await updateStaffInfo(dataToUpdate);

      // Nếu chỉ test frontend, bạn có thể bỏ API và dùng alert giả lập:
      // alert("Thông tin đã được lưu tạm thời (chưa gửi lên server).");
      setIsEditing(false);
    } catch (error) {
      setErrorMessage("Đã xảy ra lỗi khi cập nhật thông tin.");
    } finally {
      ...
      <div className="flex justify-center items-center py-10">
        <div className="bg-black/60 p-6 rounded shadow-lg w-8/10 relative ">
          ...
          <div className="flex justify-between items-center mt-8 space-x-8">
            <div className="w-1/4 flex flex-col items-center">
              <div className="w-32 h-32 bg-gray-300 rounded-full flex items-center justify-center text-4xl font-bold">
                {editedUser?.fullname?.charAt(0) || "A"}
              </div>
              <div className="mt-12 w-full">
                <button
                  onClick={() => (isEditing ? handleSave() : setIsEditing(true))}
                  className={`bg-white text-black px-4 py-3 rounded w-full cursor-pointer transition-colors`}
                  disabled={isLoading}
                >
                  {isLoading
                    ? "Đang xử lý..."
            ...
            <div className="space-y-4 w-3/5 text-black">
              <InputField
                label="Tên quản trị viên"
                name="fullname"
                value={editedUser.fullname}
