...
import { getAllEmployees, addNewEmployee, updateEmployee, deleteEmployeeById } from "../../services/employeeService";
...
const Employees = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setEmployees(data); // Đảm bảo dữ liệu từ API được gán đúng
      } catch (err) {
  ...
  const handleSaveNewEmployee = async () => {
    try {
      const newEmp = await addNewEmployee(newEmployee);
  ...
  const handleSaveEditedEmployee = async () => {
    try {
      ...
      await updateEmployee(editingEmployee.staffId, updatedData); // Gửi dữ liệu cập nhật
      setEmployees(
        employees.map((emp) =>
      ...
      <div className="p-10">
        ...
        <div className="bg-black/50 rounded-lg shadow-lg overflow-hidden w-full h-[450px]">
          <div className="flex bg-[#124035] text-white text-xl font-bold p-4">
            ...
            <div className="w-1/6 text-center">SĐT</div> */}
          ...
          <div className="p-4 overflow-y-auto max-h-[600px]">
            {employees.map((emp) => (
              ...
              >
                {/* <div className="w-1/6">{emp.staffId}</div> */}
                <div className="w-3/10">{emp.fullname}</div>
                {/* <div className="w-1/6">{emp.email}</div>
                <div className="w-1/6">{emp.phone}</div> */}
                <div className="w-3/10">{emp.position}</div>
                <div className="w-3/10">{Number(emp.salary).toLocaleString()}đ</div>
                <div className="flex space-x-2">
                  ...
                  <button
                    ...
                    onClick={() => handleDeleteEmployee(emp.staffId)}
                  >
